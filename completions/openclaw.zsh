...
_openclaw_message_send() {
  _arguments -C \
    ...
    "--channel[Channel: telegram|whatsapp|discord|irc|googlechat|slack|signal|imessage|line|feishu|openclaw-lark|nostr|msteams|mattermost|nextcloud-talk|matrix|bluebubbles|openclaw-weixin|zalo|wecom|zalouser|synology-chat|tlon]" \
...
_openclaw_agent() {
  _arguments -C \
    ...
    "--channel[Delivery channel: last|telegram|whatsapp|discord|irc|googlechat|slack|signal|imessage|line|feishu|openclaw-lark|nostr|msteams|mattermost|nextcloud-talk|matrix|bluebubbles|openclaw-weixin|zalo|wecom|zalouser|synology-chat|tlon (omit to use the main session channel)]" \
...
_openclaw_cron_add() {
  _arguments -C \
    ...
    "--channel[Delivery channel (last|feishu|openclaw-weixin|wecom)]" \
...
_openclaw_cron_edit() {
  _arguments -C \
    ...
    "--failure-alert-channel[Failure alert channel (last|feishu|openclaw-weixin|wecom)]" \
...
_openclaw_coze_upgrade_push_manifest() {
  _arguments -C \
    
}

_openclaw_coze_upgrade_apply() {
  _arguments -C \
    
}

_openclaw_coze_upgrade_status() {
  _arguments -C \
    
}

_openclaw_coze_upgrade() {
  local -a commands
  local -a options
  
  _arguments -C \
     \
    "1: :_values 'command' 'push-manifest[接收并处理升级 manifest（由 sandbox-gateway 调用）]' 'apply[执行待处理的升级计划]' 'status[查看当前升级状态]'" \
    "*::arg:->args"

  case $state in
    (args)
      case $line[1] in
        (push-manifest) _openclaw_coze_upgrade_push_manifest ;;
        (apply) _openclaw_coze_upgrade_apply ;;
        (status) _openclaw_coze_upgrade_status ;;
      esac
      ;;
  esac
}
...
_openclaw_coze() {
  ...
  _arguments -C \
     \
    "1: :_values 'command' 'upgrade[升级管理]'" \
    "*::arg:->args"

  case $state in
    (args)
      case $line[1] in
        (upgrade) _openclaw_coze_upgrade ;;
      esac
      ;;
  esac
}
...
_openclaw_openclaw_weixin() {
  ...
  _arguments -C \
     \
    "1: :_values 'command' 'logs-upload[Upload a Weixin log file to a remote URL via HTTP POST]'" \
    "*::arg:->args"
...
_openclaw_channels_capabilities() {
  _arguments -C \
    "--channel[Channel (all|telegram|whatsapp|discord|irc|googlechat|slack|signal|imessage|line|feishu|openclaw-lark|nostr|msteams|mattermost|nextcloud-talk|matrix|bluebubbles|openclaw-weixin|zalo|wecom|zalouser|synology-chat|tlon)]" \
